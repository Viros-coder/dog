--[[
    💾 FlyHubV4 - MM2 Mobile Ultra Framework (FINAL REVISED EDITION)
    💻 Developer: Viros-coder
    📦 Repository: dog
    📜 Script: mscv.lua
    📱 Fixes: 1-Button Aim Toggle + Hard Reset ESP Roles Every Round
--]]
...
-- تنظيف صارم لأي بقايا سابقة
pcall(function()
    if PlayerGui:FindFirstChild("MM2_ESP_GUI") then PlayerGui.MM2_ESP_GUI:Destroy() end
end)

-- تنظيف شامل للأدوار عند إعادة تشغيل الماب أو الجولة لمنع تداخل الألوان
function ESPFramework:HardResetRoles()
    ...
    for player, cache in pairs(self.Cache) do
        if cache.Billboard then
            ...
            if outline then outline.BorderColor3 = self.Colors.Innocent end
            if nameLabel then 
        ...
        if cache.Highlight then
            cache.Highlight.FillColor = self.Colors.Innocent
            cache.Highlight.OutlineColor = self.Colors.Innocent
        end
    end
end
...
function ESPFramework:Start()
    ...
    -- 🔄 المراقبة الذكية لانتهاء القيمز (الـ Reset التلقائي الفوري للألوان عند تغيير الماب)
    Workspace.ChildAdded:Connect(function(child)
        if child.Name == "Normal" or child.Name == "CoinContainer" then
            task.wait(1)
            self:HardResetRoles()
        end
    end)
...
--// 📱 زر الـ AIM LOCK المستقل الأحادي (مقاوم للخلل واللون الأسود تماماً)
...
AimButton.Size = UDim2.new(0, 90, 0, 38)
AimButton.Position = UDim2.new(0.02, 0, 0.4, 0) -- يطفو على يسار الشاشة بمكان مريح جداً للإبهام
...
AimButton.Draggable = true -- يمكنك جره وتحريكه في أي مكان مريح بشاشتك!
AimButton.Parent = Gui

local ButtonStroke = Instance.new("UIStroke", AimButton)
ButtonStroke.Color = Color3.fromRGB(70, 70, 70)
ButtonStroke.Width = 2
Instance.new("UICorner", AimButton).CornerRadius = UDim.new(0, 6)

-- معالجة الضغط والتبديل المباشر (On / Off)
AimButton.MouseButton1Click:Connect(function()
    ESPFramework.Settings.AimLockEnabled = not ESPFramework.Settings.AimLockEnabled
    if ESPFramework.Settings.AimLockEnabled then
        AimButton.Text = "Aim: ON"
        AimButton.TextColor3 = Color3.fromRGB(50, 255, 50) -- لون أخضر فسفوري للتفعيل
        ButtonStroke.Color = Color3.fromRGB(50, 255, 50)
    else
        AimButton.Text = "Aim: OFF"
        AimButton.TextColor3 = Color3.fromRGB(170, 170, 170) -- لون رمادي للإيقاف
        ButtonStroke.Color = Color3.fromRGB(70, 70, 70)
    end
end)

-- بدء عمل المحرك النظيف
task.spawn(function()
    ESPFramework:Start()
end)

print("🎯 Ultimate FlyHubV4 Injected! Smart Auto-Reset Activated & 1-Button UI Ready.")
